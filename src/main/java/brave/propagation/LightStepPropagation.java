...
/**
 * Implements LightStep propagation, which is a vendor-specific propagation. In case of HTTP, this
 * uses the following headers: ot-tracer-traceid, ot-tracer-spanid, ot-tracer-sampled.
 */
public class LightStepPropagation<K> implements Propagation<K> {
  public static final Factory FACTORY =
      new Factory() {
        @Override
        public <L> Propagation<L> create(final KeyFactory<L> keyFactory) {
  ...
  @Override
  public <C> TraceContext.Injector<C> injector(final Setter<C, K> setter) {
  ...
  @Override
  public <C> TraceContext.Extractor<C> extractor(final Getter<C, K> getter) {
    return new TraceContext.Extractor<C>() {
      ...
      public TraceContextOrSamplingFlags extract(final C carrier) {
        ...
        final Boolean sampled =
            sampledStr == null
                ? null
                : "true".equalsIgnoreCase(sampledStr) || "1".equals(sampledStr);
        ...
        if (result.parseTraceId(traceIdString, traceIdKey)
            && result.parseSpanId(getter, carrier, spanIdKey)) {
